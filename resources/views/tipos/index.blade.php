    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('Tipos de Equipamentos') }}
            </h2>
            <a href="{{ route('tipos.create') }}" class="inline-flex items-center px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white border border-transparent rounded-md font-semibold text-xs uppercase tracking-widest transition ease-in-out duration-150 shadow-sm">
                + Novo Tipo
            </a>
        </div>
    </x-slot>
                    ...
                    <div class="overflow-x-auto">
                        <div class="w-full text-left">
                            ...
                            <div class="flex border-b border-gray-100 dark:border-gray-700 pb-3 text-xs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">
                                ...
                                <div class="flex-1 px-4">Tipo</div>
                            ...
                            <div class="divide-y divide-gray-100 dark:divide-gray-700">
                                @forelse($tipos as $tipo)
                                    <div class="flex items-center justify-between py-3 hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition">
                                        ...
                                        <div class="flex items-center justify-end gap-3 w-44 px-4">
                                            <a href="{{ route('tipos.edit', $tipo->id) }}" class="text-xs font-bold text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300 uppercase tracking-wider">
                                                Editar
                                            </a>
                                            
                                            <form action="{{ route('tipos.destroy', $tipo->id) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja excluir este tipo? Os equipamentos vinculados a ele podem ser afetados.');" class="inline">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                    <div class="py-8 text-center text-sm text-gray-500 dark:text-gray-400">
                                        Nenhum tipo cadastrado até o momento.
                                    </div>
                                @endforelse
                            </div>
                        </div>
